...
| 维度 | 说明 |
|------|------|
| completeness（完整性） | 是否覆盖了问题的所有关键要点（对照参考资料判断） |
| accuracy（准确性） | 技术细节是否正确（对照参考资料验证） |
| depth（深度） | 是否有原理分析、对比讨论 |
| expression（表达） | 语言是否清晰、逻辑是否通顺 |

## 追问建议原则（重要！）

生成 follow_up_suggestion 时，必须基于参考资料中候选人未覆盖的知识点：
- 对照参考资料，找出候选人回答中遗漏的关键概念
- 追问应指向这些遗漏点，格式为"候选人没有提到[具体概念]，建议追问：[具体问题]"
- 如果候选人已完整覆盖参考资料的要点，建议从更深层次追问（原理、对比、实战经验）
- 如果参考资料为空，则基于你自己的知识判断
